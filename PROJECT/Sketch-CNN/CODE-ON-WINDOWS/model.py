...
from depth2ply import point_cloud_generator
...
def test_net(npr, ds, fm, fmi, cm, sm, dsm, msk, slm, cur):
    # Set logging
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            ...
            npr_1, ds_1, fm_1, fmi_1, cm_1, sm_1, dsm_1, msk_1, slm_1, cur_1 = npr, ds, fm, fmi, cm, sm, dsm, msk, slm, cur
...
def show_ply(rgb_file, depth_file, pc_file, focal_length, scalingfactor):
    a = point_cloud_generator(rgb_file, depth_file, pc_file, focal_length, scalingfactor)
    a.calculate()
    a.write_ply()
    a.show_point_cloud()
    ...
    # 以下是源代码存在的 暂时没用到
    # df = a.df
    # np.save('pc.npy',df)#存储深度信息

def do_predict(npr, ds, fm, fmi, cm, sm, dsm, msk, slm, cur):

    hyper_params['cktDir'] = ".\\savedModel"
    #hyper_params['inpDir'] = args.inpDir
    hyper_params['outDir'] = ".\\output"
    hyper_params['device'] = "0"
    hyper_params['graphName'] = "SAS_2stage_GeoNet.pbtxt"

    # Set GPU
    os.environ['CUDA_VISIBLE_DEVICES'] = "hyper_params['device']"

    # out img dir
    global out_output_img_dir,out_input_img_dir
    ...
    # Begin training
    test_net(npr, ds, fm, fmi, cm, sm, dsm, msk, slm, cur)

    # Show and store ply file
    show_ply('.\\output\\test_output_img\\fwd_normal_0.jpg', '.\\output\\test_output_img\\fwd_depth_0.jpg', 'Final_model.ply',focal_length=50, scalingfactor=1000)