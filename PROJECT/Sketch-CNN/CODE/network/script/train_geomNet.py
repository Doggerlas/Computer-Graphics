...
def collect_vis_img(logit_d, logit_n, logit_c, logit_f, npr, normal, depth, shape_mask, ds, gt_field, cline_mask, fm,
                    fm_inv, sel_m, vdotn):
    with tf.compat.v1.name_scope('collect_train_img') as _:
        ...
        cur_shape = tf.shape(input=logit_n)
    ...
    train_cline_mask_inv_proto = tf.compat.v1.summary.image('train_clmask_inv', cline_mask, hyper_params['nbDispImg'])
    ...
    return [train_npr_proto, train_gtn_proto, train_gtd_proto, train_fn_proto, train_fd_proto, train_gtds_proto,
            train_fcfm_proto, train_gt_coeff_a_proto, train_gt_coeff_b_proto, train_f_coeff_a_proto,
            train_f_coeff_b_proto, train_cline_mask_inv_proto, train_fm_proto, train_fm_inv_proto, train_selm_proto,
            train_vdotn_proto]


# TensorBoard: collect evaluating images
def collect_vis_img_val(logit_d, logit_n, logit_c, logit_f, npr, normal, depth, shape_mask, ds, gt_field, cline_mask,
                        fm, fm_inv, sel_m, vdotn):
    ...
    val_cline_mask_inv_proto = tf.compat.v1.summary.image('val_clmask_inv', cline_mask, hyper_params['nbDispImg'])
    ...
    return [val_npr_proto, val_gtn_proto, val_gtd_proto, val_fn_proto, val_fd_proto, val_gtds_proto,
            val_fcfm_proto, val_gt_coeff_a_proto, val_gt_coeff_b_proto, val_f_coeff_a_proto, val_f_coeff_b_proto,
            val_cline_mask_inv_proto, val_fm_proto, val_fm_inv_proto, val_selm_proto, val_vdotn_proto]
...
def reg_loss(logit_n, logit_d, shape_mask, cl_mask_inverse, fl_mask_inv, scope='reg_loss'):
    with tf.compat.v1.name_scope(scope) as _:
        ...
        cl_mask_crop = slice_tensor(cl_mask_inverse, logit_d)
        fl_mask_inv_crop = slice_tensor(fl_mask_inv, logit_d)
        combined_mask = shape_mask_crop * cl_mask_crop * fl_mask_inv_crop
...
def train_procedure(net, train_records, reg_weight):
    ...
    with tf.compat.v1.name_scope('divide_data'):
        ...
        gpu_ndotv = tf.split(vdotnScalar_input, nb_gpus, axis=0)
        ...
        gpu_cline_inv = tf.split(clineInvMask_input, nb_gpus, axis=0)
        ...
        for gpu_id in range(nb_gpus):
            with tf.device('gpu:%d' % gpu_id):
                with tf.compat.v1.name_scope('tower_%s' % gpu_id) as _:
                    ...
                    logit_d, logit_n, logit_c, _ = net.load_GeomNet(gpu_npr_lines[gpu_id],
                                                                    gpu_ds[gpu_id],
                                                                    gpu_mask2d[gpu_id],
                                                                    gpu_fm[gpu_id],
                                                                    gpu_selm[gpu_id],
                    ...
                    cur_trainable_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='SASMFGeoNet')
                    tower_grads.append(opt.compute_gradients(train_loss, var_list=cur_trainable_vars))
    ...
    with tf.compat.v1.name_scope('solve') as _:
        update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS, scope='.*SASMFGeoNet.*')
    ...
    return merged_train, train_op, averaged_losses[0], [npr_lines, ds, fm, fm_inv, gt_normal, gt_depth, gt_field,
...
def validation_procedure(net, val_records, reg_weight):
    ...
    # Network forward
    logit_f, field_vars = net.load_field_net(nprLine_input,
                                             mask2D_input,
                                             ds_input,
                                             fm_input,
                                             selLineMask_input,
    ...
    return merged_val, val_loss, val_d_loss, val_n_loss, val_ds_loss, val_reg_loss, val_real_dloss, \
           val_real_nloss, val_omega_loss, field_vars, [npr_lines, ds, fm, fm_inv, gt_normal, gt_depth, gt_field,
                                                        mask_cline_inv, mask_shape, mask_ds, mask2d, selm, ndotv]
...
def train_net():
    ...
    val_real_nloss, val_omega_loss, feild_vars, val_inputList = validation_procedure(net,
                                                                                     val_records,
                                                                                     reg_weight_value)
    ...
    tf_field_saver = tf.compat.v1.train.Saver(var_list=feild_vars)
        ...
        for titr in range(hyper_params['maxIter']):
                    ...
                    if eitr == idx:
                        ...
                        cur_real_dloss, cur_omega_loss, cur_real_nloss = sess.run(
                            ...
                            feed_dict={'reg_weight:0': cur_weight,
                                       ...
                                       'gtField_input:0': val_real_input[6],
                                       'clIMask_input:0': val_real_input[7],
                                       'shapeMask_input:0': val_real_input[8],
                                       'dsMask_input:0': val_real_input[9],
                                       '2dMask_input:0': val_real_input[10],
                                       'sLMask_input:0': val_real_input[11],
                                       'curvMag_input:0': val_real_input[12]
                                       })
            ...
            t_summary, _, t_loss = sess.run([train_summary, train_step, train_loss],
                                            feed_dict={'reg_weight:0': cur_weight,
                                                       ...
                                                       'gtField_input:0': train_real_input[6],
                                                       'clIMask_input:0': train_real_input[7],
                                                       'shapeMask_input:0': train_real_input[8],
                                                       'dsMask_input:0': train_real_input[9],
                                                       '2dMask_input:0': train_real_input[10],
                                                       'sLMask_input:0': train_real_input[11],
                                                       'curvMag_input:0': train_real_input[12]
                                                       })
...
if __name__ == '__main__':
    ...
    parser.add_argument('--field_ckpt', required=True, help='trained direction field checkpoint', type=str)
    ...
    hyper_params['fCktDir'] = args.field_ckpt
