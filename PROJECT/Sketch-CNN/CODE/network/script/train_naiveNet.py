...
def loss(logit_d, logit_n, normal, depth, shape_mask, ds_mask, cl_mask_inv, reg_weight, fl_mask_inv,
    ...
    with tf.compat.v1.name_scope(scope) as _:
        ...
        n_loss = tf.compat.v1.losses.mean_squared_error(gt_normal, logit_n, weights=mask_crop3)
        ...
        d_loss = tf.compat.v1.losses.mean_squared_error(gt_depth, logit_d, weights=mask_crop)
...
def train_procedure(net, train_records, reg_weight):
    ...
    train_diff_reald_proto = tf.compat.v1.summary.scalar('Training_RealDLoss', averaged_losses[3])
    train_diff_realn_proto = tf.compat.v1.summary.scalar('Training_RealNLoss', averaged_losses[4])
    train_diff_reg_proto = tf.compat.v1.summary.scalar('Training_RegL2Loss', averaged_losses[5])
    train_diff_ds_protp = tf.compat.v1.summary.scalar('Training_DsL2Loss', averaged_losses[6])
...
def train_net():
    ...
    valid_ds_loss_proto = tf.compat.v1.summary.scalar('Validating_DsL2Loss', valid_ds_loss)
